...
void HinesSolver::SynchronizeThresholdV(Branch *local, floble_t *thresholdV) {
  if (local->soma) {
    ...
    hpx_lco_set_rsync(local->branchTree->withParentLCO[6], sizeof(floble_t),
                      local->thvar_ptr);
}
...
void HinesSolver::SetupMatrixRHS(Branch *local) {
  ...
  */
  ...
  if (!local->soma)  // all top compartments except soma
  {
    ...
    hpx_lco_get_reset(neuronTree->withParentLCO[0], sizeof(floble_t),
                      &fromParentV);
    ...
    hpx_lco_set_rsync(neuronTree->withParentLCO[1], sizeof(floble_t),
                      &toParentA);
  }
  ...
  if (neuronTree != nullptr && neuronTree->branchesCount > 0) {
    ...
    for (offset_t c = 0; c < neuronTree->branchesCount; c++)
      hpx_lco_set_rsync(neuronTree->withChildrenLCOs[c][0], sizeof(floble_t),
                        &toChildrenV);

    floble_t fromChildrenA;  // rhs[p[i]] += a[i]*dv
    for (offset_t c = 0; c < neuronTree->branchesCount; c++) {
      hpx_lco_get_reset(neuronTree->withChildrenLCOs[c][1], sizeof(floble_t),
                        &fromChildrenA);
...
void HinesSolver::SetupMatrixDiagonal(Branch *local) {
  ...
  if (!local->soma)  // all branches except top
  {
    ...
    hpx_lco_set_rsync(neuronTree->withParentLCO[2], sizeof(floble_t),
                      &toParentA);
  }
  ...
  if (neuronTree != nullptr && neuronTree->branchesCount > 0) {
    ...
    {
      hpx_lco_get_reset(neuronTree->withChildrenLCOs[c][2], sizeof(floble_t),
                        &fromChildrenA);
      d[n - 1] -= fromChildrenA;
    }
  }
...
void HinesSolver::BackwardTriangulation(Branch *local) {
  ...
  if (neuronTree != nullptr && neuronTree->branchesCount > 0) {
    ...
    for (offset_t c = 0; c < neuronTree->branchesCount; c++) {
      hpx_lco_get_reset(neuronTree->withChildrenLCOs[c][3], sizeof(floble_t),
                        &fromChildrenB);
      hpx_lco_get_reset(neuronTree->withChildrenLCOs[c][4], sizeof(floble_t),
                        &fromChildrenRHS);
      d[n - 1] -= fromChildrenB;
  ...
  if (!local->soma)  // all branches except top
  {
    ...
    if (!local->soma)  // all branches except top
    {
      ...
      hpx_lco_set_rsync(neuronTree->withParentLCO[3], sizeof(floble_t),
                        &toParentB);
      hpx_lco_set_rsync(neuronTree->withParentLCO[4], sizeof(floble_t),
                        &toParentRHS);
    }
  }
}
...
void HinesSolver::ForwardSubstituion(Branch *local) {
  ...
  if (!local->soma)  // all branches except top
  {
    ...
    hpx_lco_get_reset(neuronTree->withParentLCO[5], sizeof(floble_t),
                      &fromParentRHS);
  ...
  if (neuronTree != nullptr) {
    ...
    for (offset_t c = 0; c < neuronTree->branchesCount; c++)
      hpx_lco_set_rsync(neuronTree->withChildrenLCOs[c][5], sizeof(floble_t),
                        &toChildrenRHS);
  }
}
