...
#define nrn_cur_lock _nrn_cur_lock__SK_E2
...
for (;;) { /* help clang-format properly indent */
 ...
 _PRCELLSTATE_G
  if (lock_rhs_d) lock_rhs_d(_nd_idx, type, mutex_lock_args);
         double old_rhs = _vec_rhs[_nd_idx];
         double old_d = _vec_d[_nd_idx];
        _vec_rhs[_nd_idx] -= _rhs;
	_vec_d[_nd_idx] += _g;
  if (unlock_rhs_d) unlock_rhs_d(_nd_idx, type, mutex_lock_args);
        //if (_iml<1)
        //printf("==  t=%.3f i=%d %s, gSK_E2=%.10f, gSK_E2bar=%.10f, ik=%.10f, z=%.10f, ek=%.10f, rhs=%.10f->%.10f, d=%.10f->%.10f\n",
        //       _nt->_t, _iml, _nt != &nrn_threads[0] ? "CX" : "CN", gSK_E2, gSK_E2bar, ik, z, ek, old_rhs, _vec_rhs[_nd_idx], old_d, _vec_d[_nd_idx]);
