 ...
 */
long loadDataMatrix( Info *info, char* name )
{
    long nbyte = 0;
    ...
    nbyte = sizeof(float) *(info->rowsize_*info->columnsize_);
    info->datamatrix_ = (float *) malloc(nbyte); 
...
VERBATIM
herr_t store_gid_into_vec(hid_t group_id, const char* name, void* vec) {
    int gid = atoi(name+1);
    char rebuild[32];
    snprintf(rebuild, 32, "a%d", gid);
    if (strcmp(rebuild, name) == 0) {
        vector_append(vec, (double)gid);
    }
    return 0;
}
ENDVERBATIM
...
VERBATIM {
    ...
    int result ;
    vector_resize(vec, 0);
    result = H5Giterate( info->file_, "/", NULL, store_gid_into_vec, vec );
...
VERBATIM { 
    ...
    {
        return (double)loadDataMatrix( info, gargstr(1) );
    }
    else if( ifarg(1) )
    {
        int gid = *getarg(1);
        ...
        for( gidIndex=0; gidIndex<info->confirmedCells.count; gidIndex++ ) {
            if( info->confirmedCells.gids[gidIndex] == gid ) {
                ...
                return (double)loadDataMatrix( info, cellname );
            }
        }
